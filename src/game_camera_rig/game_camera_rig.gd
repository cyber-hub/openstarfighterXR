...
@export var primary_controller : XRController3D
@onready var xr_origin_3d : XROrigin3D = $CockpitMesh/XROrigin3D
@onready var xr_Lcontroller : XRController3D = xr_origin_3d.get_node("LeftController")
@onready var xr_Rcontroller : XRController3D = xr_origin_3d.get_node("RightController")

@onready var summaryscreen := $CockpitMesh/Viewport2Din3D
#$warcity2/Viewport2DMap.get_scene_instance().zoom_map(1) #2DMap
...
func _ready():
	#$CockpitMesh/Viewport2Din3D.get_scene_instance().set_speed(100.0)
	summaryscreen.get_scene_instance().set_speed(66)
...
func _physics_process(delta: float):
	...
	Globals.joyLX = xr_Lcontroller.get_vector2("primary").x
	Globals.joyLY = -xr_Lcontroller.get_vector2("primary").y
	Globals.joyRX = xr_Rcontroller.get_vector2("primary").x
	Globals.joyRY = -xr_Rcontroller.get_vector2("primary").y
